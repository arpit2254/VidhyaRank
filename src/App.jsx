...
import { useFavorites, HeartButton, getCollegeBranchKey, FAVORITES_CSS } from "./FavoritesManager";
...
// ─── College Card ─────────────────────────────────────────────────────────────
function CollegeCard({ item, pct, cat, idx, dark, favorites, onNavigate }) {
  const cutoff    = item._rankCutoff;
  ...
  // Favorites
  const collegeKey = getCollegeBranchKey(item);
  const isFav      = favorites ? favorites.isFavorite(collegeKey) : false;
  const toggleFav  = async () => {
    if (!favorites) return;
    if (isFav) await favorites.removeFavorite(collegeKey);
    else await favorites.addFavorite(collegeKey, item);
  };
  ...
  return (
    <div ref={touchRef}
      ...
      <div style={{ padding: "14px 16px 12px" }}>
        {/* College name */}
        <h3
          onClick={() => onNavigate && onNavigate("college-detail", { college: item["College Name"] })}
          style={{ fontSize: 15, fontWeight: 800, color: "#1a0a00", lineHeight: 1.35, marginBottom: 7, fontFamily: "'Playfair Display', serif", cursor: onNavigate ? "pointer" : "default", transition: "color 0.15s" }}
          onMouseEnter={e => { if (onNavigate) e.target.style.color = "#2563eb"; }}
          onMouseLeave={e => { e.target.style.color = "#1a0a00"; }}>
          {item["College Name"]}
        </h3>
...
// ─── Results Page ─────────────────────────────────────────────────────────────
function ResultsPage({ results, pct, cat, year, capRnd, selBranches, selCities, onBack, dark, toggleDark, userBadge, favorites, onNavigate }) {
      ...
      <nav className="nav-p dm-surface dm-border" style={{ position:"sticky",top:0,zIndex:200,background:dark?"rgba(15,15,20,0.97)":"rgba(242,237,231,0.95)",backdropFilter:"blur(16px)",borderBottom:`1px solid ${dark?"#2d2a36":"#ede6dc"}`,padding:"12px 32px",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap" }}>
        ...
        {onNavigate && (
          <button
            ...
            style={{ display:"flex",alignItems:"center",gap:6,background:dark?"#252230":"#fff",color:dark?"#e0d8d0":"#374151",border:`1.5px solid ${dark?"#2d2a36":"#e5e7eb"}`,padding:"7px 14px",borderRadius:9,fontSize:13,fontWeight:600,cursor:"pointer",flexShrink:0 }}>
            ❤️ Saved
            {favorites && favorites.favorites.length > 0 && (
              <span style={{ background:"#ef4444",color:"#fff",borderRadius:"50%",width:18,height:18,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700 }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
              {group.itemsWithIdx.map(({ item, idx }) => (
                <CollegeCard
                  ...
                  favorites={favorites} onNavigate={onNavigate}
                />
              ))}
            </div>
          </div>
...
// ─── Home Page ───────────────────────────────────────────────────────────────
function HomePage({ allData, dataReady, allBranches, onSearch, dark, toggleDark, isOffline, userBadge, userBadgeDark, favorites, onNavigate }) {
        ...
        <nav className="nav-p" style={{ position:"absolute",top:0,left:0,right:0,zIndex:100,padding:"22px 44px",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            {onNavigate && (
              <button
                onClick={() => onNavigate("favorites")}
                style={{ display:"flex",alignItems:"center",gap:6,background:"rgba(255,255,255,0.1)",backdropFilter:"blur(8px)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:24,padding:"7px 16px",color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer" }}>
                ❤️ Saved
                {favorites && favorites.favorites.length > 0 && (
                  <span style={{ background:"#ef4444",color:"#fff",borderRadius:"50%",width:18,height:18,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700 }}>
                    {favorites.favorites.length}
  ...
  return (
    <div style={{ position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000 }} onClick={onClose}>
      <div style={{ background:'#fff',borderRadius:16,padding:32,maxWidth:400,width:'100%' }} onClick={e => e.stopPropagation()}>
        <h3 style={{ fontSize:20,fontWeight:700,marginBottom:20 }}>Create New Folder</h3>
        <div style={{ marginBottom:20 }}>
          <label style={{ display:'block',fontSize:14,fontWeight:600,marginBottom:8 }}>Folder Name</label>
          <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="e.g., Dream Colleges"
            style={{ width:'100%',padding:12,border:'1.5px solid #e5e7eb',borderRadius:8,fontSize:14,fontFamily:'inherit' }} autoFocus />
        </div>
        <div style={{ marginBottom:24 }}>
          <label style={{ display:'block',fontSize:14,fontWeight:600,marginBottom:8 }}>Choose Icon</label>
          <div style={{ display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:8 }}>
            {icons.map(icon => (
              <button key={icon} onClick={() => setSelectedIcon(icon)}
                style={{ padding:12,border:`2px solid ${selectedIcon===icon?'#2563eb':'#e5e7eb'}`,borderRadius:8,background:selectedIcon===icon?'#eff6ff':'#fff',fontSize:24,cursor:'pointer' }}>
                {icon}
              </button>
            ))}
          </div>
        ...
        <div style={{ display:'flex',gap:12 }}>
          <button onClick={onClose} style={{ flex:1,padding:12,border:'1.5px solid #e5e7eb',borderRadius:8,background:'#fff',fontSize:14,fontWeight:600,cursor:'pointer' }}>Cancel</button>
          <button onClick={() => { if (name.trim()) onCreate(name.trim(), selectedIcon); }} disabled={!name.trim()}
            style={{ flex:1,padding:12,border:'none',borderRadius:8,background:name.trim()?'#2563eb':'#e5e7eb',color:'#fff',fontSize:14,fontWeight:600,cursor:name.trim()?'pointer':'not-allowed' }}>
            Create
          </button>
        </div>
      </div>
...
function SavedCollegeCard({ favorite, folders, onMove, onRemove, onClick }) {
  const [showMoveMenu, setShowMoveMenu] = useState(false);
  return (
    <div style={{ background:'#fff',borderRadius:12,padding:20,border:'1px solid #e5e7eb',transition:'all 0.2s',cursor:'pointer' }}
      ...
      <div style={{ display:'flex',gap:16,alignItems:'flex-start' }}>
        <div style={{ flex:1 }} onClick={onClick}>
          <h3 style={{ fontSize:16,fontWeight:700,color:'#111827',marginBottom:8 }}>{favorite.college_name}</h3>
          <div style={{ fontSize:14,color:'#4b5563',marginBottom:8 }}>{favorite.branch_name}</div>
          <div style={{ display:'flex',gap:12,fontSize:13,color:'#6b7280' }}>
            <span>📍 {favorite.city}</span><span>•</span>
            <span>📊 {favorite.cutoff?.toFixed(2)}%</span><span>•</span>
        ...
        <div style={{ display:'flex',gap:8,alignItems:'center' }}>
          <div style={{ position:'relative' }}>
            <button onClick={e => { e.stopPropagation(); setShowMoveMenu(!showMoveMenu); }}
              style={{ padding:'6px 12px',borderRadius:6,border:'1px solid #e5e7eb',background:'#fff',fontSize:12,fontWeight:600,cursor:'pointer' }}>
              📁 Move
            </button>
            {showMoveMenu && (
              <div style={{ position:'absolute',top:'100%',right:0,marginTop:4,background:'#fff',border:'1px solid #e5e7eb',borderRadius:8,boxShadow:'0 4px 12px rgba(0,0,0,0.1)',zIndex:10,minWidth:180 }}>
                {folders.filter(f => f.id !== favorite.folder_id).map(folder => (
                  <button key={folder.id} onClick={e => { e.stopPropagation(); onMove(folder.id); setShowMoveMenu(false); }}
                    style={{ width:'100%',padding:'8px 12px',border:'none',background:'transparent',textAlign:'left',cursor:'pointer',fontSize:13,fontWeight:600,color:'#374151',display:'flex',alignItems:'center',gap:8 }}
                    onMouseEnter={e => e.currentTarget.style.background='#f9fafb'}
                    onMouseLeave={e => e.currentTarget.style.background='transparent'}>
                    <span>{folder.icon}</span><span>{folder.name}</span>
                  </button>
                ))}
              </div>
            )}
          </div>
          <button onClick={e => { e.stopPropagation(); if (window.confirm('Remove from favorites?')) onRemove(); }}
            style={{ padding:'6px 12px',borderRadius:6,border:'1px solid #e5e7eb',background:'#fff',fontSize:12,fontWeight:600,color:'#ef4444',cursor:'pointer' }}>
            ✕ Remove
          </button>
        </div>
      </div>
...
function FavoritesPage({ favorites, onNavigate, onBack, dark }) {
  const [activeFolder, setActiveFolder] = useState('all');
  const [sortBy, setSortBy]             = useState('added_at');
  const [showNewFolder, setShowNewFolder] = useState(false);

  const folderFavs = favorites.getFolderFavorites(activeFolder);
  const sortedFavorites = useMemo(() => {
    const sorted = [...folderFavs];
    switch(sortBy) {
      case 'cutoff': return sorted.sort((a, b) => a.cutoff - b.cutoff);
      case 'city':   return sorted.sort((a, b) => (a.city || '').localeCompare(b.city || ''));
      case 'name':   return sorted.sort((a, b) => (a.college_name || '').localeCompare(b.college_name || ''));
      default:       return sorted.sort((a, b) => new Date(b.added_at) - new Date(a.added_at));
    }
  }, [folderFavs, sortBy]);

  return (
    <div style={{ minHeight:'100vh',background: dark ? '#0f0f14' : '#f9fafb' }}>
      <nav style={{ background: dark ? 'rgba(15,15,20,0.97)' : '#fff', borderBottom:`1px solid ${dark?'#2d2a36':'#e5e7eb'}`, padding:'16px 32px' }}>
        <div style={{ maxWidth:1200,margin:'0 auto',display:'flex',alignItems:'center',gap:16 }}>
          <button onClick={onBack}
            style={{ padding:'8px 16px',borderRadius:8,border:`1.5px solid ${dark?'#2d2a36':'#e5e7eb'}`,background:dark?'#252230':'#fff',color:dark?'#e0d8d0':'#374151',fontSize:14,fontWeight:600,cursor:'pointer' }}>
            ← Back
          </button>
          <h1 style={{ fontSize:24,fontWeight:800,flex:1,color: dark?'#e0d8d0':'#111827' }}>My Saved Colleges</h1>
        </div>
      </nav>
      <div style={{ maxWidth:1200,margin:'0 auto',padding:32,display:'flex',gap:24 }}>
        {/* Sidebar */}
        <div style={{ width:280,flexShrink:0 }}>
          <div style={{ background:dark?'#1c1a22':'#fff',borderRadius:12,padding:20,border:`1px solid ${dark?'#2d2a36':'#e5e7eb'}`,position:'sticky',top:24 }}>
            <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16 }}>
              <h3 style={{ fontSize:16,fontWeight:700,color:dark?'#e0d8d0':'#111827' }}>Folders</h3>
              <button onClick={() => setShowNewFolder(true)}
                style={{ padding:'6px 12px',borderRadius:6,border:`1px solid ${dark?'#2d2a36':'#e5e7eb'}`,background:dark?'#252230':'#fff',color:dark?'#e0d8d0':'#374151',fontSize:12,fontWeight:600,cursor:'pointer' }}>
                + New
              </button>
            </div>
            {favorites.folders.map(folder => (
              <button key={folder.id} onClick={() => setActiveFolder(folder.id)}
                style={{ width:'100%',display:'flex',alignItems:'center',gap:10,padding:'10px 12px',border:'none',borderRadius:8,background:activeFolder===folder.id?(dark?'#2d2a36':'#f3f4f6'):'transparent',cursor:'pointer',marginBottom:4,transition:'all 0.15s' }}>
                <span style={{ fontSize:20 }}>{folder.icon}</span>
                <span style={{ flex:1,textAlign:'left',fontSize:14,fontWeight:600,color:dark?'#e0d8d0':'#374151' }}>{folder.name}</span>
                <span style={{ fontSize:12,fontWeight:700,color:'#9ca3af' }}>{folder.count}</span>
              </button>
            ))}
          </div>
        </div>
        {/* Main */}
        <div style={{ flex:1 }}>
          <div style={{ background:dark?'#1c1a22':'#fff',borderRadius:12,padding:16,border:`1px solid ${dark?'#2d2a36':'#e5e7eb'}`,marginBottom:16,display:'flex',gap:12,alignItems:'center',flexWrap:'wrap' }}>
            <span style={{ fontSize:14,fontWeight:600,color:'#6b7280' }}>Sort by:</span>
            {[{value:'added_at',label:'Recently Added'},{value:'cutoff',label:'Cutoff'},{value:'city',label:'City'},{value:'name',label:'College Name'}].map(opt => (
              <button key={opt.value} onClick={() => setSortBy(opt.value)}
                style={{ padding:'6px 12px',borderRadius:6,border:`1px solid ${dark?'#2d2a36':'#e5e7eb'}`,background:sortBy===opt.value?'#2563eb':(dark?'#252230':'#fff'),color:sortBy===opt.value?'#fff':(dark?'#e0d8d0':'#374151'),fontSize:13,fontWeight:600,cursor:'pointer' }}>
                {opt.label}
              </button>
            ))}
          </div>
          {sortedFavorites.length === 0 ? (
            <div style={{ background:dark?'#1c1a22':'#fff',borderRadius:12,padding:48,border:`1px solid ${dark?'#2d2a36':'#e5e7eb'}`,textAlign:'center' }}>
              <div style={{ fontSize:48,marginBottom:16 }}>📚</div>
              <h3 style={{ fontSize:18,fontWeight:700,marginBottom:8,color:dark?'#e0d8d0':'#111827' }}>No saved colleges yet</h3>
              <p style={{ color:'#6b7280',marginBottom:24 }}>Start exploring and click ❤️ to save colleges here.</p>
              <button onClick={() => onNavigate('home')}
                style={{ padding:'10px 24px',borderRadius:8,border:'none',background:'#2563eb',color:'#fff',fontSize:14,fontWeight:600,cursor:'pointer' }}>
                Explore Colleges
              </button>
            </div>
          ) : (
            <div style={{ display:'grid',gap:16 }}>
              {sortedFavorites.map(fav => (
                <SavedCollegeCard key={fav.id} favorite={fav} folders={favorites.folders}
                  onMove={folderId => favorites.moveToFolder(fav.college_key, folderId)}
                  onRemove={() => favorites.removeFavorite(fav.college_key)}
                  onClick={() => onNavigate('college-detail', { college: fav.college_name })} />
              ))}
            </div>
          )}
        </div>
      ...
      {showNewFolder && (
        <CreateFolderModal onClose={() => setShowNewFolder(false)}
          onCreate={(name, icon) => { favorites.createFolder(name, icon); setShowNewFolder(false); }} />
      )}
    </div>
  );
}
...
export default function App() {
  ...
  // ── Favorites ─────────────────────────────────────────────────────────────
  // Hook is declared here at top level; userId comes from session once loaded
  const [favUserId, setFavUserId] = useState(null);
  const favorites = useFavorites(favUserId);

  useEffect(() => {
    // Grab current session on mount
    supabase.auth.getSession().then(({ data: { session } }) => {
      setSession(session);
      if (session?.user?.id) setFavUserId(session.user.id);
    });
    // Listen for login / logout / token refresh events
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      setSession(session);
      setFavUserId(session?.user?.id || null);
    });
    return () => subscription.unsubscribe();
  }, []);

  // ── Navigation helper ──────────────────────────────────────────────────────
  const navigateTo = useCallback((pageName, data = {}) => {
    setPage(pageName);
    if (data.searchState) setSearchState(data.searchState);
  ...
  if (session === undefined) {
    return (
      <>
        <style>{CSS}</style>
        <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#f2ede7", flexDirection: "column", gap: 16 }}>
          <span style={{ fontSize: 24, fontWeight: 900, fontFamily: "'Playfair Display', serif", color: "#1a0a00" }}>
            Vidhya <span style={{ color: "#f97316" }}>Rank</span>
          </span>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#f97316" strokeWidth="2.5" style={{ animation: "spin 0.75s linear infinite" }}>
  ...
  // ── Logged in → show user avatar + sign-out in navs, render the app ───────
  const user = session.user;
  const avatarLetter = (user.user_metadata?.full_name || user.email || "U")[0].toUpperCase();
  const avatarUrl    = user.user_metadata?.avatar_url;

  // Inject user avatar pill into HomePage and ResultsPage via a wrapper
  const UserBadge = ({ inDark }) => (
    <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 10px 4px 4px", borderRadius: 30, background: inDark ? "#252230" : "rgba(255,255,255,0.12)", border: `1px solid ${inDark ? "#2d2a36" : "rgba(255,255,255,0.18)"}`, backdropFilter: "blur(8px)" }}>
        {avatarUrl
          ? <img src={avatarUrl} alt="" style={{ width: 26, height: 26, borderRadius: "50%", objectFit: "cover", flexShrink: 0 }} />
          : <div style={{ width: 26, height: 26, borderRadius: "50%", background: "#f97316", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 800, color: "#fff", flexShrink: 0 }}>{avatarLetter}</div>
        }
        <span style={{ fontSize: 12, fontWeight: 600, color: inDark ? "#e0d8d0" : "rgba(255,255,255,0.9)", maxWidth: 110, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
      ...
      <button
        ...
        title="Sign out"
        style={{ padding: "6px 12px", borderRadius: 8, border: `1px solid ${inDark ? "#2d2a36" : "rgba(255,255,255,0.2)"}`, background: inDark ? "#1c1a22" : "rgba(255,255,255,0.1)", color: inDark ? "#998f88" : "rgba(255,255,255,0.7)", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", backdropFilter: "blur(8px)", transition: "all 0.15s" }}
        onMouseEnter={e => { e.currentTarget.style.background = "#dc2626"; e.currentTarget.style.color = "#fff"; e.currentTarget.style.borderColor = "#dc2626"; }}
        onMouseLeave={e => { e.currentTarget.style.background = inDark ? "#1c1a22" : "rgba(255,255,255,0.1)"; e.currentTarget.style.color = inDark ? "#998f88" : "rgba(255,255,255,0.7)"; e.currentTarget.style.borderColor = inDark ? "#2d2a36" : "rgba(255,255,255,0.2)"; }}
      >
        Sign out
      </button>
    </div>
    ...
    <>
      ...
      {page === "home" && (
        <HomePage
          ...
          userBadge={<UserBadge inDark={false} />}
          userBadgeDark={<UserBadge inDark={true} />}
          favorites={favorites} onNavigate={navigateTo}
        />
      )}
      {page === "results" && searchState && (
        <ResultsPage
          ...
          userBadge={<UserBadge inDark={dark} />}
          favorites={favorites} onNavigate={navigateTo}
        />
      )}
      {/* Favorites page */}
      {page === "favorites" && (
        <FavoritesPage
          ...
          onBack={() => navigateTo(searchState ? "results" : "home")}
          dark={dark}
        />
      )}
      {/* College detail page */}
      {page === "college-detail" && selectedCollege && (
        <CollegeDetailPage
